...
import { RoomUpdatesSubscription, RoomUpdatesSubscriptionVariables } from "src/gql/gqlTypes/RoomUpdatesSubscription";
import { UpdateType } from "src/gqlGlobalTypes";

const WrRoomDetail = (): JSX.Element => {
  const { roomId } = useParams<{ roomId: string }>();

  const { data, error, subscribeToMore } = useQuery<RoomDetailQuery, RoomDetailQueryVariables>(ROOM_DETAIL_QUERY, {
    variables: { id: roomId },
  });
  useEffect(() => {
    subscribeToMore<RoomUpdatesSubscription, RoomUpdatesSubscriptionVariables>({
      ...
      variables: { id: roomId },
      updateQuery(prev, { subscriptionData: { data: { roomUpdates } } }) {
