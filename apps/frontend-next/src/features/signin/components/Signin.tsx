  ...
  return (
    <Group>
      ...
      {process.env.NODE_ENV === 'development' && (
        <>
          <TextInput value={name} onChange={(e) => setName(e.currentTarget.value)} label="dev. signin name" />
          <Button onClick={() => developmentSignin(name)}>Sign in in Development</Button>
        </>
      )}
    </Group>
  );
