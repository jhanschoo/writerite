...
export interface MessageUpdatePublishArgs {
  [MESSAGE_UPDATES_BY_ROOM_SLUG_TOPIC]: [
    slug: string,
    payload: MessageUpdateBase
  ];
}
...
export const MessageCreateMutation = mutationField(MESSAGE_CREATE_KEY, {
  ...
  resolve: guardValidUser(
    async (_source, { slug, type, content }, { prisma, pubsub, sub }) => {
...
export const MessageUpdatesByRoomSlugSubscription = subscriptionField(
