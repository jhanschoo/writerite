...
export const RoomSetDeckMutation = mutationField(ROOM_SET_DECK_KEY, {
  ...
  resolve: guardValidUser(
    async (_parent, { id: roomId, deckId }, { prisma, pubsub, sub }) => {
      const roomRes = await roomSetDeck(prisma, {
        roomId,
        deckId,
...
export const RoomSetStateMutation = mutationField(ROOM_SET_STATE_KEY, {
  ...
  resolve: guardValidUser(
    async (_parent, { id, state }, { prisma, pubsub, redis, sub }) => {
...
export const RoomUpdatesByRoomSlugSubscription = subscriptionField(
  "roomUpdatesByRoomSlug",
  {
