  ...
  describe("Query", () => {
    describe("user", () => {
      it("should be able to fetch all user-accessible fields of current user", async () => {
        ...
        const currentUserBefore = unsafeJwtToCurrentUser(createUserResponse.data.finalizeThirdPartyOauthSignin as string);

        // login as user
        setSub(currentUserBefore);

        // query own user
        const queryUserRequest = await queryAllUserAccessibleUserScalars(server, currentUserBefore.id);
