...
import LineEditor, {
  lineEditorStateFromString,
} from "src/components/editor/LineEditor";
...
const WrDeckDetailData = ({ deck, readOnly }: Props): JSX.Element => {
  ...
  const [editorState, setEditorState] = useState(
    lineEditorStateFromString(deck.name)
  );
  ...
  const [mutateUsed] = useMutation<DeckUsedMutation, DeckUsedMutationVariables>(
    DECK_USED_MUTATION,
    {
      variables: {
        id: deck.id,
  ...
  const [mutateEdit, { loading }] = useMutation<
    DeckEditMutation,
    DeckEditMutationVariables
  >(DECK_EDIT_MUTATION, {
    onCompleted(data) {
  ...
  const [mutateRoomCreate] = useMutation<
    RoomCreateMutation,
    RoomCreateMutationVariables
  >(ROOM_CREATE_MUTATION, {
  ...
  const handleChange = (newEditorState: EditorState) => {
    ...
    if (title) {
      ...
      return EditorState.push(
        newEditorState,
        ContentState.createFromText(currentTitle),
        "insert-characters"
      );
    }
