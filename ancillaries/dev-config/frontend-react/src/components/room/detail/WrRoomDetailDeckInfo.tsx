...
const WrRoomDetailDeckInfo = ({ deckId }: Props): JSX.Element | null => {
  const { data } = useQuery<DeckQuery, DeckQueryVariables>(DECK_QUERY, { variables: { id: deckId } });
  ...
  return <DeckBox>
    ...
    <DeckInfoBox>
      ...
      <DeckStatistics>
        {`used ${moment.duration(moment.utc(usedAt).diff(now)).humanize()} ago`}
        <br />
        {`edited ${moment.duration(moment.utc(editedAt).diff(now)).humanize()} ago`}
      </DeckStatistics>
    </DeckInfoBox>
  </DeckBox>;
};
