...
import {
  DECKS_QUERY,
  DECK_DETAIL_QUERY,
  OWN_DECK_RECORD_QUERY,
  ROOM_QUERY,
} from "src/gql";
import {
  OwnDeckRecordQuery,
  OwnDeckRecordQueryVariables,
...
export const deckCreateMutationUpdate: MutationUpdaterFn<DeckCreateMutation> = (
  cache,
  { data }
) => {
  ...
  if (newDeck) {
    ...
    try {
      const decksQuery = {
        query: DECKS_QUERY,
        variables: { scope: DecksQueryScope.OWNED, titleFilter: "" },
      };
      const decksData = cache.readQuery<DecksQuery, DecksQueryVariables>(
        decksQuery
      );
...
export const cardCreateMutationUpdate: MutationUpdaterFn<CardCreateMutation> = (
  cache,
  { data }
) => {
  ...
  if (newCard) {
    ...
    try {
      ...
      const deckDetailData = cache.readQuery<
        DeckDetailQuery,
        DeckDetailQueryVariables
      >(deckDetailQuery);
      ...
      const oldCards = newCard.mainTemplate
        ? deck.cards?.map(
            (card) => card && { ...card, mainTemplate: card.id === newCard.id }
          )
        : deck.cards;
...
export const cardEditMutationUpdate: MutationUpdaterFn<CardEditMutation> = (
  cache,
  { data }
) => {
    ...
    try {
      ...
      cache.writeQuery<DeckDetailQuery, DeckDetailQueryVariables>({
        ...
        data: {
          ...
          deck: {
            ...deckDetailData.deck,
            cards: deckDetailData.deck.cards.map(
              (card) =>
                card && { ...card, mainTemplate: card.id !== newCard.id }
            ),
          },
        },
      });
...
export const cardDeleteMutationUpdate: MutationUpdaterFn<CardDeleteMutation> = (
  cache,
  { data }
) => {
    ...
    try {
      ...
      cache.writeQuery<DeckDetailQuery, DeckDetailQueryVariables>({
        ...
        data: {
          ...
          deck: {
            ...deckDetailData.deck,
            cards: deckDetailData.deck.cards.filter(
...
export const roomCreateMutationUpdate: MutationUpdaterFn<RoomCreateMutation> = (
  cache,
  { data }
) => {
...
export const OWN_DECK_RECORD_SET_MUTATION = gql`
  ${USER_DECK_RECORD_SCALARS}
  mutation OwnDeckRecordSetMutation($deckId: ID!, $notes: JSONObject) {
    ownDeckRecordSet(deckId: $deckId, notes: $notes) {
...
export const ownDeckRecordSetMutationUpdate: MutationUpdaterFn<
  OwnDeckRecordSetMutation
> = (cache, { data }) => {
