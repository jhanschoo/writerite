...
export const agent =
  httpUrl.protocol === "https:" && wsUrl.protocol === "wss:"
    ? new https.Agent({
        keepAlive: true,
        ca: !CA_CERT_FILE ? undefined : fs.readFileSync(CA_CERT_FILE),
      })
...
if (agent === undefined) {
  throw new Error(
    "GRAPHQL_HTTP and GRAPHQL_WS are inconsistent; their protocols need to be either https: and wss: or http: and ws:"
  );
}
