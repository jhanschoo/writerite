...
import { Box, Button, Card, LoadingOverlay, LoadingOverlayProps, MantineTheme, Stack, Text, TextInput, UnstyledButton, useMantineTheme } from "@mantine/core"
...
const addSubdeckProps: LoadingOverlayProps = {
  visible: true,
  loader: <Stack align="center" spacing={1}><Link2Icon width="40" height="40" /><Text size="xl" weight="bold">Link</Text></Stack>
};
...
const addedProps = ({ background, color }: ReturnType<MantineTheme["fn"]["variant"]>): LoadingOverlayProps => ({
  onClick(e) { e.stopPropagation(); },
  visible: true,
  loader: <Stack align="center" spacing={1}><CheckIcon width="40" height="40" color={color} /><Text color={color} size="xl" weight="bold">Added</Text></Stack>,
  overlayColor: background,
});

const DeckItem = ({ parentId, added, onAdded }: DeckItemFactoryProps): FC<DeckItemProps> => ({ deck }) => {
  ...
  return (
    ...
    >
      <Card.Section ref={cardRef}>
        <UnstyledButton sx={{ position: 'relative' }} component="div" p="md" onClick={handleAddSubdeck}>
          <LoadingOverlay
      ...
      />
      <Card.Section>
        <NextLinkButton to={`/app/deck/${deck.id}`} fullWidth variant="subtle" sx={{ borderRadius: 0 }}>View</NextLinkButton>
      </Card.Section>
    </Card>
  );
};
...
export const ManageDeckSubdecksAddSubdeck: FC<ManageDeckProps> = ({ deck: { id: deckId, subdecks } }) => {
  ...
  return <Stack p="md" spacing={2}>
    <TextInput
      ...
      mb="md"
      value={titleFilter}
      onChange={(e: ChangeEvent<HTMLInputElement>) => { setTitleFilter(e.target.value); setAdded([]); }}
    />
    <DecksList decks={candidateDecks} component={DeckItem({ parentId: deckId, added, onAdded })} />
  </Stack>;
}
