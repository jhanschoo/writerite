...
const DeckItem: (parentId: string) => FC<DeckItemProps> = (parentId) => ({ deck }) => {
      ...
      <UnstyledButton sx={{ height: 'unset' }} component="div">
        ...
        >
          <Card.Section inheritPadding pt="sm">
            <Button size="xs" radius="xs" compact rightIcon={<LinkBreak2Icon />} variant="filled" className={classes.cardCloseButton} disabled={fetching} onClick={handleCardDelete}>
              remove
            </Button>
          </Card.Section>
...
export const ManageDeckSubdecksBrowse: FC<ManageDeckProps> = ({ deck: { id: deckId, subdecks } }) => {
