...
export const ManageDeckCards: FC<ManageDeckProps> = ({ deck }) => {
  ...
  return (
    <Tabs orientation="vertical" value={activeTab} onTabChange={setActiveTab}>
      <Tabs.List>
        <Tabs.Tab value="browse" icon={<MagnifyingGlassIcon />} aria-label="Browse Cards">{matches && "Browse Cards"}</Tabs.Tab>
        <Tabs.Tab value="upload" icon={<UploadIcon />} arial-label="Import from file">{matches && "Import from file"}</Tabs.Tab>
      </Tabs.List>
